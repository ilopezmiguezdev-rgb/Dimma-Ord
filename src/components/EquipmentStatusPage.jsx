...
const getEquipmentStatus = (equipment, clientServiceOrders) => {
...
const SortableEquipmentCard = ({ equipment, onUpdate, onDelete, serviceOrders }) => {
  ...
  const { status, message, color } = getEquipmentStatus(equipment, serviceOrders);
        ...
        <div className="p-2 bg-slate-100 dark:bg-slate-900/50 rounded-b-lg flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
          <Popover>
            ...
            <PopoverContent className="w-60 p-3 text-xs space-y-2">
              ...
              {equipment.sub_clients && (
                <div>
                  <span className="font-semibold text-slate-600 dark:text-slate-300">Lab: </span>
                  <span>{equipment.sub_clients.name}</span>
                </div>
              )}
            </PopoverContent>
          </Popover>
          {canDelete && <Button variant="ghost" size="icon" className="h-6 w-6 text-red-500 hover:text-red-600" onClick={() => setDeleteDialogOpen(true)}><Trash2 className="h-4 w-4" /></Button>}
        </div>
      </div>
...
const ClientEquipmentSection = ({ client, equipment, serviceOrders, onUpdate, onDelete, onClientSelect, onDeleteClient, labFilter }) => {
  const [isDeleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const { canDelete } = usePermissions();
        ...
        {clientEquipment.length > 0 ? (
            <SortableContext items={clientEquipment.map(e => e.id)} strategy={verticalListSortingStrategy}>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                {clientEquipment.map((equip) => <SortableEquipmentCard key={equip.id} equipment={equip} onUpdate={onUpdate} onDelete={onDelete} serviceOrders={clientServiceOrders} />)}
              </div>
            </SortableContext>
        ) : (
...
const EquipmentStatusPage = ({ clients, equipment, serviceOrders, deliveries, loading, onEquipmentUpdate, onClientSelect }) => {
  ...
  useMemo(() => {
    const newMap = clients.reduce((acc, client) => {
      acc[client.id] = equipment.filter(e => e.sub_clients?.client_id === client.id);
      return acc;
    }, {});
  ...
  const availableLabs = useMemo(() => {
    ...
    clientEquip.forEach(e => {
      if (e.sub_clients) labMap.set(e.sub_clients.id, e.sub_clients.name);
    });
  ...
  const handleDragEnd = async ({ active, over }) => {
    ...
    const targetClientId = Object.keys(clientEquipmentMap).find(
      clientId => clientEquipmentMap[clientId].some(e => e.id === over.id) || clientId === over.id
    );
    if (!targetClientId) return;
    const { error } = await supabase
      .from('equipment_inventory')
      .update({ client_id: targetClientId, sub_client_id: null })
      .eq('id', active.id);
    if (error) {
      toast({ title: "Error", description: "No se pudo mover el equipo.", variant: "destructive" });
    } else {
      onEquipmentUpdate();
    }
  };
  ...
  const handleDeleteClient = async (clientId) => {
    const { data: clientSubClients } = await supabase.from('sub_clients').select('id').eq('client_id', clientId);
    if (clientSubClients && clientSubClients.length > 0) {
      const subClientIds = clientSubClients.map(sc => sc.id);
      const { error: deleteEquipError } = await supabase.from('equipment_inventory').delete().in('sub_client_id', subClientIds);
      if (deleteEquipError) {
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="p-2 md:p-4">
        ...
        {visibleClients && visibleClients.length > 0 ? (
          <div className="space-y-6">
            {visibleClients.map((client) => ( <ClientEquipmentSection key={client.id} client={client} equipment={clientEquipmentMap[client.id] || []} serviceOrders={serviceOrders} onUpdate={onEquipmentUpdate} onDelete={handleDeleteEquipment} onClientSelect={onClientSelect} onDeleteClient={handleDeleteClient} labFilter={labFilter}/> ))}
          </div>
        ) : (
